...
namespace SmartGeoLogical.Render.Bore.Column
{
    ...
    public class Description:Control
    {
        ...
        public Description(RectangleF rect, Color outLineColor, Color fillColor, bool drawOutLine, bool fillInner, string name)
        ...
        public AlignStyle Align { get => align; set => align = value; }
        ...
        private void SetBottom(LayerInfo layerInfo, Graphics g,int index)
        {
            ...
            SizeF s = g.MeasureString(str, this.fontD);
        ...
        private void drawDescription(LayerInfo layer,int index,Graphics g)
        {
            ...
            if (bottoms[index] > primitiveBottom[index])   //需要拉线
            {
                ...
                g.DrawString_2(GetText(layer), this.fontD, new SolidBrush(Color.Black), new RectangleF(Left+Width/20, topElevation+(bottomElevation - topElevation)/20, Width*9/10, bottomElevation - topElevation), GetFormat(this.align));
                g.DrawLine(new Pen(Color.Black), new PointF(Left, primitiveBottom[index]), new PointF(Left + Width /20, bottomElevation));
                g.DrawLine(new Pen(Color.Black), new PointF(Left + Width / 20, bottomElevation), new PointF(Left + Width- Width / 20, bottomElevation));

                g.DrawLine(new Pen(Color.Black), new PointF(Left + Width - Width / 20, bottomElevation), new PointF(Left + Width, primitiveBottom[index]));
            }
            else                                           //不需要拉线
            {
                bottomElevation = primitiveBottom[index];
                g.DrawString_2(GetText(layer), this.fontD, new SolidBrush(Color.Black), new RectangleF(Left + Width / 20, topElevation, Width*9/10, bottomElevation - topElevation), GetFormat(this.align));
