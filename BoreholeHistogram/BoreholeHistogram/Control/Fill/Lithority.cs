...
namespace BoreholeHistogram.Control.Fill
{
    /// <summary>
    /// 岩性纹理
    /// </summary>
     class Lithority:Control
    {
        ...
        private void DrawLayer(LayerInfo layer, float totalDepth,float holeElevation,Graphics g, string legend)
        {
            ...
            float layerHeight = Math.Abs( layer.LayerBottom - tempTop)*this.Rect.Height/totalDepth;
            //float layerTop = this.Top+Math.Abs(tempTop - holeElevation) * this.Height / totalDepth;
            float layerTop = this.Rect.Top + Math.Abs(tempTop-beginDepth) * this.Rect.Height / totalDepth;
            ...
            g.DrawImage(img, new RectangleF(this.Rect.Left, layerTop, this.Rect.Width, layerHeight));
            g.DrawRectangle(new Pen(Color.Black), this.Rect.Left, layerTop, this.Rect.Width, layerHeight);
        ...
        private Image RepeatImag(float layerHeight, Image img)
        {
            ...
            int xCount = (int)Math.Round(this.Rect.Width /50);
            int yCount = (int)Math.Round(layerHeight / 30);
            xCount = xCount != 0 ? xCount : 1;
            yCount = yCount != 0 ? yCount : 1;
            if (!(xCount == 1 && yCount == 1))
            {
                int singleWidth = (int)Math.Ceiling(this.Rect.Width / xCount);
                int singleHeigth = (int)Math.Ceiling(layerHeight / yCount);
              
                Size size = new Size((int)this.Rect.Width, (int)layerHeight);
                Bitmap bmp = new Bitmap((int)this.Rect.Width, (int)layerHeight);
